...
// getPath Returns the handler registered with the given path (key). The values of
		...
		if len(path) > len(n.Path) {
			if path[:len(n.Path)] == n.Path {
				...
				case RouteNodeTypeParam:
					...
					p = append(p, RouteParameter{
						Key: n.Path[1:], Value: path[:end],
					})
